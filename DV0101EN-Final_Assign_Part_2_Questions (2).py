...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
       # unemp_veh_sal = recession_data.groupby()
        R_chart4  = dcc.Graph(figure=px.bar(recession_data, 
                                    x=pd.cut(recession_data['unemployment_rate'],8), 
                                    y='Automobile_Sales', 
                                    color = 'Vehicle_Type', 
...
#plot 1 Yearly Automobile sales using line chart for the whole period.
        ...
        Y_chart1 = dcc.Graph(figure=px.line(yas, 
                                     x='Year',
                                     y='Automobile_Sales',
                                title="Yearly Average Automobile Sales in the year {}".format(input_year) ))
...
# Plot 2 Total Monthly Automobile sales using line chart.
        ...
        Y_chart2 = dcc.Graph(figure=px.line(total_monthly, 
                                    x= 'Month', 
                                    y='Automobile_Sales', 
                                    title="Monthly Total Automobile Sales in the year {}".format(input_year) ))
        ...
        #avr_vdata.set_index('Vehicle_Type', inplace=True)
        #avr_vdata = avr_vdata.groupby('Month')['Automobile_Sales']  #might need to remove the .mean()
        Y_chart3 = dcc.Graph(figure=px.bar(avr_vdata, 
                                    #x='Vehicle_Type', 
                                    x='Month',
                                    y='Automobile_Sales', 
                                    title="Average number of vehicles sold in the year {}".format(input_year)))
                                    # Average Monthly Vehicle Sales by Vehicle Type
